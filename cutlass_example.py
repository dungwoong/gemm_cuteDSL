...
class HopperWgmmaGemmKernel:
    ...
    def __init__(
        ...
        # NOTE number of times we have to multicast
    ...
    def _setup_attributes(self):
        ...
        # NOTE not sure why, but they make it so we execute 4 MMAs along the k dim, and set tile shape like that.
        # I guess this is just to make the tiles sufficiently large(?)
        ...
        # NOTE this is getting the number of pipeline stages for AB, and stages for epilogue. Warps(?) will take turns writing epilogue to SMEM and then to GMEM I'm guessing
    ...
    def __call__(
        ...
        # TODO I haven't checked these out in detail though
    ...
    def kernel(
        ...
        cluster_id = cidx + cdimx * cidy # NOTE the id of the cluster, like blockID. We can plan our own rasterization of clusters now

        # CTA Swizzle to promote L2 data reuse
        # NOTE seems like they're making a layout for the clusters...
        group_size_m = 8
        s_shape = (
        ...
        # RASTERIZATION ##########################################################
        # NOTE this groups the clusters into groups of 8, and then goes down in the row direction then columns
        ...
        cluster_coord_mnk = cta_layout_mnk.get_flat_coord(cta_rank_in_cluster) # you can use CTA layout to change the layout of CTAs in the cluster

        # ///////////////////////////////////////////////////////////////////////////////
        # Get mcast mask
        # ///////////////////////////////////////////////////////////////////////////////
        # NOTE educated guess, you mask out the CTAs you're not supposed to multicast to
        ...
        # mbar arrays
        # NOTE this is defined in the struct, this is how we access the data pointer
        mainloop_pipeline_array_ptr = storage.mainloop_pipeline_array_ptr.data_ptr()


        # NOTE CooperativeGroups, look at cute cooperative groups(I think) ######################################
        ...
        mainloop_pipeline = pipeline.PipelineTmaAsync.create(
            ...
            defer_sync=True, # so we can sync later
        )

        #  Cluster arrive after barrier init
        # NOTE this initializes the barriers and then arrives at a cluster-wide barrier. Mbarrier is managed behind the scenes(?)
        # they do pipeline_init_wait later.
        ...
        # NOTE composed layouts have an outer layout and an inner layout
        # it's (inner o offset o outer) so inner is the swizzle, outer is the layout
        ...
        # NOTE Make C by recasting sC pointer, and then make a tensor using this I guess
        # Not really sure why they do it this way
        ...
        # NOTE you only have to partition C because A and B come from SMEM 
        ...
        a_cta_layout = cute.make_layout(cute.slice_(cta_layout_mnk, (0, None, 0)).shape) # literally like 2 or something
        a_cta_crd = cluster_coord_mnk[1] # It's the multicast dim
        sA_for_tma_partition = cute.group_modes(sA, 0, 2)
        gA_for_tma_partition = cute.group_modes(gA_mkl, 0, 2) # not sure why this is required

        # TODO this is important to work out, not sure why they group modes and stuff
        # but this is basically just partitioning the TMA across each tensor for later copying
    ...
    def _compute_stages(
        ...
        NOTE:
        Stages for ABC = pipeline depth
        They also give stages for epilogue, such that they can reuse ABC SMEM for it.
...
def run(
    ...
    **kwargs,
    ...
    print(mA, a_torch.shape)
    print(mB, b_torch.shape)
    print(mC, c_torch.shape)
